...
export default function Clothes_child() {
  ...
  const { openModal, Modal, closeModal } = useModal(false);
  return (
    <div>
      <div className=" max-w-screen-xl mx-auto">
        <h3 className="text-start font-bold text-2xl my-10">
          Vêtements à la une
        </h3>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4">
            ...
            >
              <div>
                ...
                <div
                  id={`dots-${product1.id}`}
                  className={`${
                    dotOpen[product1.id] ? "block" : "hidden"
              ...
              <h3 className=" text-red-700 font-medium absolute ">
                {product1.prix}$
              </h3>
          ...
          {/*Modal form*/}
          <Modal>
            <ModalAddArticle closeModal={closeModal} />
          </Modal>
        </div>
        <div>
          <Link
          ...
          >
            ...
            >
              <path
                d="M3 14.5H37M37 14.5L22.4286 3M37 14.5L22.4286 26"
                stroke="#DC3545"
                strokeWidth="5"
                strokeLinecap="round"
                strokeLinejoin="round"
        ...
        {/* List_for_clothes-child2 */}
        <div>
          <h3 className=" text-start font-bold text-2xl my-10 mt-20">
            Autres Vêtements
          </h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4">
            {products_child2.map((product2) => (
              <div
                className="bg-gray-100 p-4 rounded-lg shadow-md relative"
                key={product2.id}
              >
                <div>
                  ...
                  <div
                    id={`dots-${product2.id}`}
                    className={`${
                      dotOpen[product2.id] ? "block" : "hidden"
                    } bg-white text-base list-none absolute divide-y divide-gray-100 rounded shadow my-4 md:w-44 w-[90%]`}
                  >
                    <ul
                      className="py-1"
                      aria-labelledby={`dots-${product2.id}`}
                    >
                      <li>
                        <Link
                          to="/updateproduct"
